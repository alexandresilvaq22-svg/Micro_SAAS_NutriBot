  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center relative overflow-hidden transition-transform hover:scale-[1.02] duration-300">
      <div className="w-full flex justify-between items-start mb-2">
        <h3 className="text-slate-500 font-semibold text-sm uppercase tracking-wider">{data.name}</h3>
      ...
      <div className="text-center mt-2">
        <div className="text-2xl font-bold text-slate-800">
          {data.current} <span className="text-sm font-normal text-slate-400">/ {data.target}{data.unit}</span>
        </div>
        <p className="text-xs font-medium text-slate-500 mt-1">
          Falta <span className="text-slate-700 font-bold">{remaining}{data.unit}</span> para a Meta
        </p>
      </div>
